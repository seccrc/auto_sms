# -*- coding: utf-8 -*-
"""
윈도우 "휴대폰 연결"(Phone Link, 구 "당신의 휴대폰") 앱을 pywinauto로 조작한다.

⚠ 중요: 이 파일의 컨트롤 탐색 셀렉터(자동화ID/컨트롤타입/이름)는 실제 화면을
inspect 해보지 못한 상태에서 UWP 메시지 앱들의 일반적인 구조를 추정해 작성한
"최선의 추측"이다. capture_location_auto.py가 국민신문고 목록/상세 화면의
버튼을 여러 후보로 시도했던 것과 같은 이유 — 정확한 이름을 실제로 확인 못
했으니, 실제 윈도우 PC에서 아래 순서로 한 번 맞춰봐야 한다.

    1. 휴대폰 연결 앱을 켜고 메시지 화면을 열어둔다.
...
# 앱 창 제목 — 한국어("휴대폰 연결")와 영어("Phone Link") 둘 다 커버.
WINDOW_TITLE_RE = r".*(휴대폰\s*연결|Phone Link|Your Phone).*"
...
# 메시지 입력창(하단 텍스트박스). 보통 Edit 컨트롤타입.
_COMPOSE_BOX_CRITERIA = dict(control_type="Edit")
# 보내기 버튼. 이름이 "보내기"/"Send"/아이콘만 있을 수 있어 여러 후보를 시도한다.
_SEND_BUTTON_NAME_CANDIDATES = ["보내기", "Send", "전송"]
_NEW_MESSAGE_BUTTON_NAME_CANDIDATES = ["새 메시지", "New message", "새 채팅"]


def _connect_main_window(timeout: int = 15):
    """이미 실행 중인 휴대폰 연결 앱 창에 붙는다. 앱이 안 떠 있으면
    ...
    try:
        ...
        raise RuntimeError(
            "휴대폰 연결 앱 창을 찾지 못했습니다. 앱이 실행 중이고 휴대폰과 "
...
def _find_send_button(container):
    for name in _SEND_BUTTON_NAME_CANDIDATES:
        try:
            btn = container.child_window(title=name, control_type="Button")
            if btn.exists(timeout=1):
                return btn
        except Exception:
            continue
    # 이름으로 못 찾으면 컨트롤타입만으로 마지막 버튼(보통 보내기가 입력창
    # 오른쪽 끝에 있음)을 시도 — 최후의 수단.
    try:
        ...
        pass
    raise RuntimeError("보내기 버튼을 찾지 못했습니다. --dump로 실제 이름을 확인해주세요.")
...
def send_message(phone_number: str, body: str):
    ...
    send_btn = _find_send_button(win)
